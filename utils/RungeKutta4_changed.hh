...
/** @file **/
#ifndef RUNGEKUTTA4_CHANGED_HH_
#define RUNGEKUTTA4_CHANGED_HH_
...
void runge_kutta_fixed4_changed(RHS rhs, state_type &x, const state_type &x_const, input_type &u, const int dim, const double tau, const int nint=10) noexcept {
  state_type k[4];
  state_type tmp;
  ...
  for(int t=0; t<nint; t++) {
    ...
    rhs(k[1],tmp,x_const, u);
    for(int i=0;i<dim;i++)
      tmp[i]=x[i]+h/2*k[1][i];

    rhs(k[2],tmp,x_const, u);
    for(int i=0;i<dim;i++)
      tmp[i]=x[i]+h*k[2][i];

    rhs(k[3],tmp,x_const, u);
    for(int i=0; i<dim; i++)
