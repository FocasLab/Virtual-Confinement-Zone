...
reach_states=(t_r2(1)-1)*n_y+t_r2(2);
...
controlled_Delta2=logical(sparse(n_x*n_y*n_u,n_x*n_y));
for i=1:(n_x*n_y)
    for h=1:(n_u)
        if(Cont(i,h))
            controlled_Delta2((i-1)*n_u+h,:) = Delta((i-1)*n_u+h,:);
        end
    end
end
...
save('Controlled_System2','controlled_Delta2');
save('Target_2', 't_r2');

% disp(winning_domain)
init_pos = winning_domain(1,1:2);
