...
def controller_transition(Delta,Cont):
    controlled_Delta2 = np.zeros((n_x*n_y*n_u,n_x*n_y),dtype=bool) 
    Delta2 = Delta.copy()
    for i in range(n_x*n_y):
        print("Progress:- ", (i / (n_x*n_y)) * 100)
        for h in range(n_u):
            if(Cont[i][h]):
                controlled_Delta2[i*n_u+h][:] = Delta2[i*n_u+h][:]
            else:
                break
    return controlled_Delta2
...
controlled_Delta2 = controller_transition(control_delta,reach_controller)
savemat("D:/Git codes/Sym_MAS/sym_MAS/Simple Discrete System/controlled_Delta2.mat",{'controlled_Delta2':controlled_Delta2})