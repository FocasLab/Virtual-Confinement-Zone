...
[delta1, win_domain1, T1] = SimpleDiscreteSystem(r, states, inputs, lb_states, ub_states, lb_inputs, ub_inputs, s_eta, i_eta, target1, obstacles);
[delta2, win_domain2, T2] = SimpleDiscreteSystem(r, states, inputs, lb_states, ub_states, lb_inputs, ub_inputs, s_eta, i_eta, target2, obstacles);
...
delta1_idxs = find(delta1);
delta2_idxs = find(delta2);
...
for d1_idx=1:size(delta1_idxs, 1)
    ...
    for d2_idx=1:size(delta2_idxs, 1)
        ...
        if(norm([points1(3:-1:2)] - [points2(3:-1:2)], inf)-d-L*eta_max*0.5>=0)
            B_prime = norm([s_points1(2:-1:1)] - [s_points2(2:-1:1)], inf) - norm([points1(3:-1:2)] - [points2(3:-1:2)], inf);
            if (B_prime >= -0.9*(norm([points1(3:-1:2)] - [points2(3:-1:2)], inf)-d-L*eta_max*0.5))
...
T1_idxs = find(T1);
T2_idxs = find(T2);
for t1_idx=1:size(T1_idxs, 1) % compute the whole interval contained into the extremal intervals
    [st1, ~] = ind2sub(size(T1), T1_idxs(t1_idx));
