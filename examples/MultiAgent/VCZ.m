%% Generate VCZ trajectory using abstraction-based synthesis

clc;
clear all;
close all;

%%%% FOR THE DYNAMICS xi(k+1)=xi(k)+u(k) %%%%
...
%obstacles = [[1.5, 3.5, 4.5, 6.5]; [11.5, 3.5, 14.5, 6.5]; [5.5, 11.5, 10.5, 14.5]];
obstacles = [[1, 4, 4, 7]; [8, 4, 11, 7]];
...
%% Abstraction
[m1, s1] = memory;   % before running
tic

disp(' ')
disp('[VCZ]:Step 1- Computation of the abstraction.')
disp(' ')
...
disp(' ')
disp('[VCZ]:Step 2 - Controller Synthesis.')
disp(' ')
...
disp(' ')
disp('[VCZ]:Step 4 - Comuputation of Transition System and Winnig Domain')
disp(' ')
...
[m2, s2] = memory;   % after running

%% Printing time
fprintf("[VCZ]:Total Abstraction time: %2.4f\n", abs_time);
fprintf("[VCZ]:Total Synthesis time: %2.4f\n", syn_time);
fprintf("[VCZ]:Total Run time: %2.4f\n", abs_time + syn_time);
disp(['Memory used: ', num2str(m2.MemUsedMATLAB - m1.MemUsedMATLAB), ' bytes']);

%% Simulation
% Current pose index
X_curr = [9.25, 9.25, 5.25, 1.25];
...
writematrix([poses; f_targets; f_obstacles],'poses.csv');
%% Values to Plot
dom = find(Controller);
[r,~] = ind2sub(size(Controller), dom); % states
r = unique(r);
[dom_control,~] = size(r); % domain of controller
nzc = nnz(Controller); % no. of non-zero elements in the controller
total_tim = abs_time + syn_time; % total run time
